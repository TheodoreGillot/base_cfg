...
if [[ "$OS" == "mac-arm64" ]]; then
    ...
    mv nvim-macos-arm64 "$BIN_DIR/"
    chmod u+x "$BIN_DIR/nvim-macos-arm64"
    ln -sf "$BIN_DIR/nvim-macos-arm64" "$BIN_DIR/nvim"
fi

if [[ "$OS" == "mac-x86_64" ]]; then
    curl -LO https://github.com/neovim/neovim/releases/download/v0.10.4/nvim-macos-x86_64.tar.gz
    tar -xzf nvim-macos-x86_64.tar.gz
    mv nvim-macos-x86_64 "$BIN_DIR/"
    chmod u+x "$BIN_DIR/nvim-macos-x86_64"
